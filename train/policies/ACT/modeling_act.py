...
class ACTPolicy(nn.Module):
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        if len(self._action_queue) == 0:
            ...
            if self.config.control_mode in ControlMode.get_candidate("task"):
                pos_rot_hat = decode_6d_action_head(actions[:, :, :(3 + 6) * self.config.num_robots], num_robots=self.config.num_robots)
        ...
        return {"actions": self._action_queue.popleft(), "success_probability": self._success_prob}
    ...
    def forward(self, batch: dict[str, Tensor]) -> dict[str, Tensor]:
        ...
        # forward
        output, (mu_hat, log_sigma_x2_hat) = self.model(batch)
        actions_hat = output["actions"]
        if self.config.use_success_detector:
            success_prob_hat = output["success_probability"]
        
        # apply orthogonalization of task space control
        if self.config.control_mode in ControlMode.get_candidate("task"):
            pos_rot_hat = decode_6d_action_head(actions_hat[:, :, :(3 + 6) * self.config.num_robots], num_robots=self.config.num_robots)
                
            if self.config.robot_mode in [RobotMode.SINGLE_ROBOT, RobotMode.DUAL_ROBOT]:
                actions_hat = pos_rot_hat
            else:
                extra_action_hat = actions_hat[:, :, (3 + 6) * self.config.num_robots:]  # gripper, control dt, success
                actions_hat = torch.concat((pos_rot_hat, extra_action_hat), dim=-1)
...
class ACT(nn.Module):
    ...
    def __init__(self, config: ACTConfig):
        ...
        if "observation.state" in config.input_shapes:
            input_state_shape = config.input_shapes["observation.state"][0]
        else:
            input_state_shape = sum(v[0] for k, v in config.input_shapes.items() \
        ...
        if self.config.use_vae:
            ...
            if self.use_input_state:
                self.vae_encoder_robot_state_input_proj = nn.Linear(
                    input_state_shape, config.dim_model
                )
            # Projection layer for action (joint-space target) to hidden dimension.
            self.vae_encoder_action_input_proj = nn.Linear(
                config.action_dim, config.dim_model
            )
        ...
        # Must before generating success detector!
        # Especially when using different initializer for the success detector.
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        if self.config.use_vae and "action" in batch:
            ...
            if self.config.control_mode in ControlMode.get_candidate("task"):
                parsed_action = batch["action"][:, :, :3 * self.config.num_robots]
                for robot_id in range(self.config.num_robots):
                    parsed_action = \
                        torch.cat([parsed_action, batch["action"][:, :, 3 * self.config.num_robots + 9 * robot_id: 3 * self.config.num_robots + 9 * robot_id + 6]], dim=-1)
                if self.config.robot_mode in [RobotMode.SINGLE_ROBOT_GRIPPER, RobotMode.DUAL_ROBOT_GRIPPER]:
                    parsed_action = torch.cat([parsed_action, batch["action"][:, :, -self.config.num_robots:]], dim=-1)
                
                action_embed = self.vae_encoder_action_input_proj(parsed_action)  # (B, S, D)
            else:
            ...
            key_padding_mask = torch.cat(
                [cls_joint_is_pad, batch["is_pad"]], axis=1
            )  # (bs, seq+1 or 2)
        ...
        if self.config.use_success_detector:
            output["success_probability"] = success_prob

        return output, (mu, log_sigma_x2)
